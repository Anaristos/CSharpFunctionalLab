...
namespace Demo
{
    class BankCard
    {
        ...
        {
            if (at.CompareTo(this.ValidBefore) >= 0) return 0;

            return Math.Min(Balance, desired);
        }
    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            decimal available1 = card.GetAvailableAmount(20, queryTime); // returns 20

            decimal available2 = card.GetAvailableAmount(20, queryTime.AddSeconds(3)); // returns 0

            decimal available3 = card.GetAvailableAmount(20, queryTime); // returns 20
